...
export default async function recordResult(points, result) {
  ...
  try {
    ...
    if(commits.length < 2 || commits.length && commits[0].author && commits[0].author.login.includes('[bot]') || process.env.IS_ORIGINAL_TEMPLATE_REPO || repository.is_template) return
  } catch (error) {
    console.log(error)
  }
  ...
  const payload = JSON.stringify({
    ...
    TEST_RUNTIME_ERRORS: removeTerminalColoring(result.testResults[0].message), // TEXT
